...
function Invoke-PlotDotnet {
    [CmdletBinding(SupportsShouldProcess)] param([hashtable]$Settings,$Context)
    $directory=Get-AdapterPath $Settings.Directory $Context
    $target=Get-AdapterPath $Settings.Project $Context
    if (-not (Test-Path -LiteralPath $target)) { throw 'Project or solution does not exist.' }
    if ($Settings.Command -notin @('restore','build','test','publish','pack')) { throw 'Unsupported dotnet command.' }
    if ($PSCmdlet.ShouldProcess($target,('dotnet '+$Settings.Command))) {
        Invoke-AdapterProcess $Settings $Context 'dotnet' (@($Settings.Command,$target)+@($Settings.Arguments)) $directory
    }
}
function Invoke-PlotNuget {
    [CmdletBinding(SupportsShouldProcess)] param([hashtable]$Settings,$Context)
    $directory=Get-AdapterPath $Settings.Directory $Context
    if ($Settings.Command -notin @('push','delete','list','locals','add','remove','update','enable','disable')) { throw 'Unsupported dotnet nuget command.' }
    if ($PSCmdlet.ShouldProcess($directory,('dotnet nuget '+$Settings.Command))) {
        Invoke-AdapterProcess $Settings $Context 'dotnet' (@('nuget',$Settings.Command)+@($Settings.Arguments)) $directory
    }
}
Export-ModuleMember -Function Invoke-PlotDotnet,Invoke-PlotNuget
