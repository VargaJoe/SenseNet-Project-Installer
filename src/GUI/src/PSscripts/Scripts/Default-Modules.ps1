...
Function Module-Stop {
	...
	$ProjectSiteName = $ProjectSettings.IIS.WebAppName
	 & "$scriptpath\Ops\Stop-IISSite.ps1" $ProjectSiteName
}
...
<#
	...
	#>
	$ProjectSiteName = $ProjectSettings.IIS.WebAppName
	& "$scriptpath\Ops\Start-IISSite.ps1" $ProjectSiteName
}
...
<#
	...
	& $scriptpath\Dev\GetLatestSolution.ps1 -tfexepath "${env:ProgramFiles(x86)}\Microsoft Visual Studio 14.0\Common7\IDE\tf.exe" -location "$ProjectSourceFolderPath"
...
<#
	...
	#>
	$ProjectSolutionFilePath = Get-FullPath $ProjectSettings.Project.SolutionFilePath
	& $scriptpath\Dev\Build-Solution.ps1 -slnPath $ProjectSolutionFilePath
}
...
<#
	...
	& $scriptpath\Deploy\Tool-Module.ps1 -ToolName "install-services" -ToolParameters "datasource:$DataSource","initialcatalog:$InitialCatalog","FORCEDREINSTALL:true"
}
...
<#
	...
	& $scriptpath\Deploy\Tool-Module.ps1 -ToolName "install-webpages" 
}
...
<#
	...
	$RemoveDemoPackagePath = Get-FullPath ".$scriptpath\Packages\RemoveDemo"
	& $scriptpath\Deploy\Package-Module.ps1 "$RemoveDemoPackagePath"
	
	# $RemoveDemoPackagePath = Get-FullPath ".$scriptpath\Packages\RemoveDemo\import"
	# & $scriptpath\Deploy\Import-Module.ps1 "$RemoveDemoPackagePath"
}
...
<#
	...
	& $scriptpath\Deploy\Package-Module.ps1 "$ProjectDeployFolderPath"	
}
...
<#
	...
	& $scriptpath\Ops\Create-IISSite.ps1 -DirectoryPath $ProjectWebFolderPath -SiteName $ProjectSiteName -PoolName $ProjectAppPoolName -SiteHosts $ProjectSiteHosts
}
...
<#
	...
	#>
	$ProjectSiteHosts = $ProjectSettings.IIS.Hosts
	& $scriptpath\Ops\Set-Host.ps1 -SiteHosts $ProjectSiteHosts
}
...
<#
	...
	& $scriptpath\Tools\Unzip-File.ps1 -filename "$SnWebfolderPackName" -destname "$SnWebfolderPackPath"
}
...
<#
	...
	#>
	& iisreset
	& $scriptpath\Deploy\Index-Project.ps1 
}
...
<#
	...
	& $scriptpath\Deploy\Import-Module.ps1 "$ProjectStructureFolderPath"
}
...
<#
	...
	if (!($Exportfromfilepath)){
		...
		& $scriptpath\Deploy\Export-Module.ps1 "$ProjectWebFolderPath\App_Data\Export$CurrentDateTime"
	}else{
		Write-Host "Start export script by filter: $Exportfromfilepath"
		& $scriptpath\Deploy\Export-Module.ps1 "$ProjectWebFolderPath\App_Data\Export$CurrentDateTime" "$ExportFilter"
	}
}
...
<#
	...
	Write-Host $scriptpath\Create-Package.ps1 -SourceRootPath "$ProjectSourceFolderPath" -TargetRootPath "$ProjectSourceFolderPath/Packages"
	& $scriptpath\Create-Package.ps1 -SourceRootPath "$ProjectSourceFolderPath" -TargetRootPath "$ProjectSourceFolderPath/Deployment/Packages"
}
